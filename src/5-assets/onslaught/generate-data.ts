...
const RarityStringSchema = z.enum([
	"Common",
	"Uncommon",
	"Rare",
	"Epic",
	...
	.strictObject({
		...
		battles: z
			...
			.refine((zone) => new Set(zone.map((b) => b.boardId)).size === 1, {
				message:
					"all zones within a sector are expected to share the same BoardId",
			})
			.refine((zone) => new Set(zone.map((b) => b.badgeAlliance)).size === 1, {
				message:
					"all zones within a sector are expected to reward badges from the same alliance",
			}),
	})
	.transform((sector) => ({
		minHeroPower: sector.minHeroPower,
		badgeAlliance: sector.battles[0].badgeAlliance,
		killzones: sector.battles.map(
			({ badgeAlliance, boardId, ...rest }, index) => ({
...
export type OnslaughtData = z.infer<typeof DataSchema>;
